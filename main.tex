...
\documentclass[14pt,aspectratio=169]{beamer}  % <-- draft entfernt
\usetheme{metropolis}
...
\begin{tikzpicture}[node distance=1.1cm]
  \node[box,text width=2.9cm] (txt){log line};
  \node[box,right=2.6cm of txt,text width=2.9cm] (vec){sparse vector};
  \draw[arr](txt)--(vec);
\end{tikzpicture}
\end{frame}
...
\begin{itemize}[<+->]
  ...
  \item Rare / unusual points are isolated \emph{early}.  
        Path length \(h(x)\) $\rightarrow{}$ anomaly score  
        \(s(x)=2^{-h(x)/c(n)}\).
  \item CPU-friendly: 30 µs per line, no GPU.
\end{itemize}

\column{0.45\linewidth}
\centering
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[scale=.9,every node/.style={font=\scriptsize}]
  \draw[fill=gray!15] (0,0) rectangle (3,3);
  \node at (1.5,3.2){feature 1};
  ...
  \node[anchor=west] at (3.2,1.4){few splits $\rightarrow$ high score};
\end{tikzpicture}
\end{adjustbox}
\end{columns}
\end{frame}
...
\begin{enumerate}[<+->]
  \item \IForest says \emph{“something odd”}. Operators still ask \emph{“why?”}
  \item \RForest majority vote assigns one of seven error categories.
  \item Per-token feature importance allows future visual highlights.
  \item Nightly training < 90 s (400 trees, depth 30).
\end{enumerate}
...
\begin{adjustbox}{max width=1.3\linewidth, max height=.9\textheight}
